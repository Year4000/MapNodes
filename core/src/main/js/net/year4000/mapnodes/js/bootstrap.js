/*
 * Copyright 2017 Year4000. All Rights Reserved.
 */
'use strict'

// Utils Shaded with Gradle
include('/modules/lodash.js')
include('/modules/immutable.js')
include('/modules/EventEmitter.js')
include('/modules/logger.js')
include('/modules/three.js')
include('/modules/moment.js')

// The files needed for MapNodes
include('/net/year4000/mapnodes/js/colors.js')
include('/net/year4000/mapnodes/js/utils.js')
include('/net/year4000/mapnodes/js/conditions.js')
include('/net/year4000/mapnodes/js/facts.js')
include('/net/year4000/mapnodes/js/injection.js')
include('/net/year4000/mapnodes/js/messages.js')
include('/net/year4000/mapnodes/js/events/event_manager.js')
include('/net/year4000/mapnodes/js/command/cmd_errors.js')
include('/net/year4000/mapnodes/js/command/cmd_executor.js')
include('/net/year4000/mapnodes/js/command/cmd_manager.js')
include('/net/year4000/mapnodes/js/mapnodes.js')
include('/net/year4000/mapnodes/js/command/commands.js')
include('/net/year4000/mapnodes/js/regions/abstract_region.js')
include('/net/year4000/mapnodes/js/regions/point_region.js')
include('/net/year4000/mapnodes/js/regions/cuboid_region.js')
include('/net/year4000/mapnodes/js/regions/cube_region.js')
include('/net/year4000/mapnodes/js/regions/global_region.js')
include('/net/year4000/mapnodes/js/regions/regions.js')
include('/net/year4000/mapnodes/js/game/json_object.js')
include('/net/year4000/mapnodes/js/game/player.js')
include('/net/year4000/mapnodes/js/game/team.js')
include('/net/year4000/mapnodes/js/game/game.js')
include('/net/year4000/mapnodes/js/game/kit.js')
include('/net/year4000/mapnodes/js/game/region.js')
include('/net/year4000/mapnodes/js/game/clazz.js')
include('/net/year4000/mapnodes/js/game/game_mode.js')
