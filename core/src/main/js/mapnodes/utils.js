...
export const point_and_look = (x, y, z, point_to) => {
  // todo fix the trig?
  const height = Math.max(x, point_to.point.x) - Math.min(x, point_to.point.x)
  const yaw_width = Math.max(z, point_to.point.z) - Math.min(z, point_to.point.z)
  const yaw = Math.atan(yaw_width / height) * 180 / Math.PI
  const pitch_width = Math.max(y, point_to.point.y) - Math.min(y, point_to.point.y)
